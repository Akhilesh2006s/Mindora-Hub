...
const EnhancedLessonManagementScreen: React.FC<EnhancedLessonManagementScreenProps> = ({ navigation, route }) => {
  ...
  const saveAllLessonsToBackend = async () => {
    try {
      ...
      try {
        const existingLessons = await apiService.get('/lessons');
      ...
      Alert.alert(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: async () => {
              try {
                const response = await apiService.post('/lessons/sync-to-modules', {});
  ...
  const handleAddTopic = async () => {
    ...
    try {
      const response = await apiService.post(`/lessons/${selectedLesson._id}/topics`, {
